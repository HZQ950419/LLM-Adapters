...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=["boolq", "piqa", "social_i_qa", "hellaswag", "winogrande", "ARC-Challenge", "ARC-Easy", "openbookqa", "sciq"],
                        required=True)
...
def extract_answer(args, sentence: str) -> float:
    ...
    if dataset == 'boolq':
        ...
        if not pred_answers:
            return ""
        return pred_answers[0]
    elif dataset in ['social_i_qa', 'ARC-Challenge', 'ARC-Easy', 'openbookqa', 'sciq']:
        sentence_ = sentence.strip()
